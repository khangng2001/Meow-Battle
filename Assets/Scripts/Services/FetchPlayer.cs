        ...
        private async Task FetchPlayersInRoom(string lobbyID)
        {
            try
            {
                ...
                foreach (Player player in lobby.Players)
                {
                    ...
                    if (player.Data["ReadyState"].Value == "Ready") playerExisted.transform.GetChild(2).gameObject.SetActive(true);
                    else playerExisted.transform.GetChild(2).gameObject.SetActive(false);
        ...
        private async void FetchFriendsInRoom(string lobbyID)
        {
            try
            {
                ...
                foreach (string f in playerDetails.listfriends)
                {
                    ...
                    friendsListManager.EmitUpdateStatus(friend.status, friend.unityID); // Chỉ hoạt động với những lần showlistfriend chạy lại từ đầu
